...
class HelloKittyCalculator:
    ...
    def setup_buttons(self):
        # Grid definition for simple cute calculator
        # Colors match reference image 2:
        # - Number keys: White rounded heads with pink text
        # - Operations (+, -, ×, ÷, %): Soft Pastel Blue (#82B3F4)
        # - Clear/Del: Soft Coral (#FF7597)
        # - Equals (=): Vibrant Hello Kitty Red (#FF2A55)
    ...
    def create_3d_button(self, label, btn_type, x1, y1, x2, y2):
        # Color palettes based on button type
        if btn_type == "num":
            bg_color = "#FFFFFF"
            shadow_color = "#E0C0C8"
            text_color = "#FF4D6D"
    ...
    def render_button_graphics(self, b):
        # Clean previous items if redrawing
    ...
    def handle_button_press(self, label):
        # Trigger Hello Kitty Bow wiggle on button press!
    ...
    def update_lcd(self):
        ...
        # Scale down font size if text is very long
    ...
    def animate(self):
        # 1. Animate floating background particles
        for p in self.particles:
            p['y'] -= p['speed_y']
        ...
        # 2. Animate Hello Kitty Eye Blink
        self.blink_counter += 1
        if self.blink_counter > 140:  # Blink every ~4.5 seconds
